(function($, window) {
    let ele = null,
        ...
        g = {}, //全局变量
        defaults = {
            "navWidth": 92, //列表每个宽度,该版本中请把宽高填写成一样
            "navHeight": 92, //列表每个高度,该版本中请把宽高填写成一样
            "navItemNum": 5, //列表显示个数
...
<p class='exzoom_preview'>
    ...
    function move(direction) {
        ...
        let nav_item_width = g.navItemMargin + g.navWidth + g.navBorder * 5; // 单个导航元素的宽度
    ...
    function getImageSize(url, callback) {
        ...
        if (typeof callback !== "undefined") {
            if (img.complete) {
                callback(img.width / 2, img.height / 2);
            } else {
                // 完全加载完毕的事件
                img.onload = function() {
                    callback(img.width / 2, img.height / 2);
    ...
    //     var t = e.pageY - $(".small_box").offset().left - ($(".float_layer").height() / 2) + 200;
    //     if (l < 0) {
    //         l = 0
    //     }
    //     if (l > ($(this).width() - $(".float_layer").width()) * 1.5) {
    //         l = ($(this).width() - $(".float_layer").width()) * 1.5
    //     }
    //     if (t < 0) {
    //         t = 0
    //     }
    //     if (t > ($(this).height() - $(".float_layer").height()) * 1.5) {
    //         t = ($(this).height() - $(".float_layer").height()) * 1.5
    //     }
    //     $(".float_layer").css({
    //         "left": l,
    //         "top": t
